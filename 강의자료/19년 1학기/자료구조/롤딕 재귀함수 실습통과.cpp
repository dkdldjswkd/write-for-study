...
void champserch(lol_dic champ[], char name2[], int i) { // 1번 챔피언 정보 보기
...
void insert(lol_dic champ[], int i) { // 2번 챔피언 정보 입력
	if (i == num) {
...
void champdel(lol_dic champ[], char name[], int i) {// 3번 챔피언 정보 삭제
	if (i == 100) return;
...
void delposi(lol_dic champ[], char pos[], int i) { // 4번 입력 포지션 챔피언 정보 삭제
	if (i == 100)return;
...
void printall(lol_dic champ[], int i) { // 5번 챔피언 전부 출력
	if (i == 100) return;
	if (champ[i].name[0] != ' ') {
		champ[i].show();
	}
...
int best = 0;
void printbest(lol_dic champ[], int index, int i) { // 6번 가장 체력이 높은 챔피언 출력
	if (i == 100) {
		champ[index].show();
		return;
	}
...
void sort(lol_dic champ[], int i) { // 7번 챔피언 체력 높은순으로 재정렬
	if (i == 100)return;
	lol_dic tmp;
	for (int j = i + 1; j < 100; j++) {// 체력이 높은 챔피언 순서대로 교환 알고리즘
